...
<div class="main-content">
    ...
    <div class="content-header">
        <div class="d-flex justify-content-between align-items-center" style="position:relative;z-index:1;">
            <div>
                ...
                <p class="content-subtitle">Manage and track employee payroll</p>
            </div>
            <a href="{{ route('employees.create') }}" class="cu-new-btn">
    ...
    {{-- ── Stat Strip ──────────────────────────────────────────── --}}
    @php
        $totalProjects    = $employees->count();
        $activeProjects   = $employees->where('status', 'in_progress')->count();
        $doneProjects     = $employees->where('status', 'completed')->count();
        $overdueProjects  = $employees->filter(fn($p) => $p->end_date && $p->end_date->isPast() && !in_array($p->status, ['completed','closed']))->count();
    @endphp
    ...
    @if($employees->count() > 0)
        ...
        <div class="cu-grid" id="cuGrid">
            @foreach($employees as $employee)
                @php
                    ...
                    $projectColor   = $colors[strlen($employee->name) % count($colors)];
                    $chipClass      = match($employee->status) {
                        'in_progress' => 'in-progress',
                        'completed'   => 'completed',
                        'closed'      => 'closed',
                        default       => 'not-started'
                    };
                    $chipLabel = match($employee->status) {
                        'in_progress' => 'In Progress',
                        'completed'   => 'Completed',
                        'closed'      => 'Closed',
                        default       => 'Not Started'
                    };
                    $teamCount = $employee->teamMembers ? $employee->teamMembers->count() : 0;
                    $isOverdue = $employee->end_date && $employee->end_date->isPast() && !in_array($project->status, ['completed','closed']);
                @endphp
                    ...
                    <div class="cu-card-body">
                        <div class="cu-card-top">
                            ...
                            <div class="flex-grow-1 min-w-0">
                                <div class="cu-card-name" title="{{ $employee->name }}">{{ $employee->name }}</div>
                                <div class="cu-card-desc">{{ strip_tags($employee->description) ?: 'No description' }}</div>
                            <div><span title="{{ $employee->salary }}">Net Pay : {{ ($employee->salary + $employee->allowance) - (($employee->salary + $employee->allowance)*0.05) }}</span></div>
                            </div>
                        ...
                        <div class="cu-card-actions">                      
                            <a href="{{ route('employees.show', $employee) }}" class="cu-action-btn" title="View"><i class="bi bi-printer"></i></a>
                            <a href="{{ route('employees.edit', $employee) }}" class="cu-action-btn" title="Edit"><i class="bi bi-pencil"></i></a>
        ...
        <div class="cu-list" id="cuList">
            <div class="cu-list-header">
                <div>Name</div>
                <div class="col-meta">Position</div>
                <div class="col-meta">Net Salary (RM)</div>
                <div class="col-meta">Basic Salary (RM)</div>
                <div class="col-meta">Allowance (RM)</div>
                <div></div>
            </div>
            @foreach($employees as $employee)
                @php
                    ...
                    $colors         = ['#6366f1','#8b5cf6','#06b6d4','#10b981','#f59e0b','#ef4444','#ec4899'];
                    $projectColor   = $colors[strlen($employee->name) % count($colors)];
                    $chipClass      = match($employee->status) { 'in_progress'=>'in-progress','completed'=>'completed','closed'=>'closed',default=>'not-started'};
                    $chipLabel      = match($employee->status) { 'in_progress'=>'In Progress','completed'=>'Completed','closed'=>'Closed',default=>'Not Started'};
                    $isOverdue      = $employee->end_date && $employee->end_date->isPast() && !in_array($employee->status, ['completed','closed']);
                @endphp
                <div class="cu-list-row project-item"
                    ...
                    <div class="cu-list-name">
                        <div class="cu-list-avatar" style="background:{{ $projectColor }};">{{ strtoupper(substr($employee->name,0,1)) }}</div>
                        <span title="{{ $employee->name }}">{{ $employee->name }}</span>
                    </div>
                    <div><span title="{{ $employee->position }}">{{ $employee->position }}</span></div>
                    <div><span title="{{ $employee->salary }}">{{ ($employee->salary + $employee->allowance) - (($employee->salary + $employee->allowance)*0.05) }}</span></div>
                    <div><span title="{{ $employee->salary }}">{{ $employee->salary }}</span></div>
                    <div><span title="{{ $employee->allowance }}">{{ $employee->allowance }}</span></div>
    ...
    @else
        <div class="cu-empty">
            ...
            <h5>No name in the list</h5>
            <p>Update the Employee List.</p>
            <a href="{{ route('employees.create') }}" class="cu-empty-btn"><i class="bi bi-plus-lg"></i>Add New Employee</a>
        </div>
    @endif
