...
@section('title', $employee->name . ' — Project Details')
...
<div class="main-content">
    ...
    <div class="content-header">
        <div class="d-flex align-items-center" style="position:relative;z-index:1;">
            <a href="{{ route('employees.index') }}" class="me-3 text-decoration-none">
                <i class="bi bi-arrow-left fs-5" style="color:rgba(255,255,255,.8);"></i>
            </a>
            <div>
                <h1 class="content-title mb-0">{{ $employee->name }}</h1>
    ...
    <div class="cu-layout">
        ...
        <div class="cu-left-panel">
            <div class="cu-panel-top">
                ...
                <div class="cu-panel-name">{{ $employee->name }}</div>
             </div>
            ...
            <div class="cu-panel-rows">
                <div class="cu-panel-row">
                    ...
                    <span class="cu-panel-row-value">
                        {{ $employee->start_date ? $employee->start_date->format('M d, Y') : '—' }}
                    </span>
                </div>        
                <div class="cu-panel-row">
                    <span class="cu-panel-row-label"><i class="bi bi-people"></i> Position</span>
                    <span class="cu-panel-row-value">{{ $employee->position }}</span>
                </div>
                <div class="cu-panel-row">
                    <span class="cu-panel-row-label"><i class="bi bi-clock-history"></i> Updated</span>
                    <span class="cu-panel-row-value">{{ $employee->updated_at->diffForHumans() }}</span>
                </div>
            </div>

            <div class="cu-panel-actions">
                <a href="{{ route('employees.edit', $employee) }}" class="cu-panel-action-btn primary">
                    <i class="bi bi-pencil"></i> Edit Employee Info
                </a>
                <button type="button" class="cu-panel-action-btn danger" onclick="confirmDelete()">
                    <i class="bi bi-trash"></i> Remove Employee
                </button>
                <form id="deleteForm" action="{{ route('employees.destroy', $employee) }}" method="POST" class="d-none">
                    @csrf @method('DELETE')
                </form>
            </div>
        </div>
        ...
        <div>
            ...
            <div class="cu-left-panel">
                <div class="cu-section-header">
                    ...
                    <span class="cu-section-title">Payslip (in RM)</span>
                </div>
                <div class="cu-panel-rows">
                <div class="cu-panel-row">
                    <span class="cu-panel-row-label"><i class="bi bi-cash"></i> Basic Salary</span>
                    <span class="cu-panel-row-value">
                        {{ $employee->salary }}
                    </span>
                </div>
                <div class="cu-panel-row">
                    <span class="cu-panel-row-label"><i class="bi bi-coin"></i> Allowance</span>
                    <span class="cu-panel-row-value">
                        {{ $employee->allowance }}
                    </span>
                </div>
                <div class="cu-panel-row">
                    <span class="cu-panel-row-label"><i class="bi bi-cash-coin"></i> Gross Pay</span>
                    <span class="cu-panel-row-value">{{ ($employee->salary + $employee->allowance) }}</span>
                </div>
                <div class="cu-panel-row">
                    <span class="cu-panel-row-label"><i class="bi bi-bank"></i> Tax (5%)</span>
                    <span class="cu-panel-row-value">{{ ($employee->salary + $employee->allowance)*0.05 }}</span>
                </div>
                <div class="cu-panel-row">
                    <span class="cu-panel-row-label"><i class="bi bi-wallet"></i> Net Pay</span>
                    <span class="cu-panel-row-value">{{ ($employee->salary + $employee->allowance) - (($employee->salary + $employee->allowance)*0.05) }}</span>
                </div>
                ...
                <div class="cu-section-body">
                    ...
                    @else
                        <p class="mb-0" style="color:#8a8f98;font-size:13px;font-style:italic;">
                            ...
                            <a href="{{ route('employees.edit', $employee) }}" style="color:#7c3aed;">Add one →</a>
                        </p>
                    @endif
                </div>
            </div>
...
<div class="modal fade" id="addMemberModal" tabindex="-1" aria-labelledby="addMemberModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form action="{{ route('employees.addMember') }}" method="POST">
                @csrf
                <input type="hidden" name="project_id" value="{{ $employee->id }}">
...
function confirmDelete() {
    if (confirm('Delete "{{ addslashes($employee->name) }}"? All tasks, files and data will be permanently removed. This cannot be undone.')) {
